...
def accuracy_plot_save(accuracy_list,epochs,accuracy_best,epoch_acuracy_best,save_path):
    ax1 = plt.subplots()
    ax1.plot(epochs,accuracy_list,color="blue")
    ax1.set_xlabel('epoch')
    ax1.set_ylabel('accuracy')
    ax1.scatter(epoch_acuracy_best, accuracy_best,color="red", marker="*") # この行は前回の回答と同じ
    ax1.annotate('({},{})'.format(str(epoch_acuracy_best),str(accuracy_best)), xy=(epoch_acuracy_best,accuracy_best*0.999))# この行からテキストと矢印を追加
    plt.savefig(str(save_path/'accuracy.jpg'), dpi=150, bbox_inches='tight')
...
def resave(train_names,valid_names, result_matrix,save_dir,back_img,nmatcls_img,subt_img,save_inc_img,iou_conf_scatter):
